...
def dec_file():
    ...
    if applied_language == 'en':
        keytext = easygui.enterbox("Enter the Secret Key for Decryption", 'File Keeper')
    if applied_language == 'kr':
    ...
    try:
        ...
        info_area.config(state='normal')
        if applied_language == 'en':
            info_area.insert('end',"Failed to Decrypt.\n")
            messagebox.showinfo("Error", "File decryption is failed.\nPlease check the password you entered, the file extension, and key length seeting.")
        if applied_language == 'kr':
            info_area.insert('end',"복호화 실패.\n")
            messagebox.showinfo("Error", "파일 복호화 실패.\n입력하신 비밀번호, 파일 확장자, 키 길이 설정을 다시 확인 해주시길 바랍니다.")    
        info_area.config(state='disabled')
...
info_area.insert('end','                                                           version 1.2\n')
